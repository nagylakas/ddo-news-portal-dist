(function () {
    ...
    function render(el,p) {
        ...
        } else if (!p.logged_in && p.status==='active') {
            h+='<div class="poll-login-prompt" role="note"><i class="bi bi-person-lock poll-login-icon" aria-hidden="true"></i><div><p class="poll-login-title">A szavazáshoz bejelentkezés szükséges</p><p class="poll-login-text">Jelentkezz be, majd add le a szavazatodat.</p><a class="btn btn-primary poll-login-button" href="'+esc(p.login_url)+'"><i class="bi bi-box-arrow-in-right me-2" aria-hidden="true"></i>Bejelentkezés és szavazás</a></div></div>';
        }
