...
document.addEventListener('DOMContentLoaded', function() {
	// Keep the article cover image moving independently inside its fixed hero
	// frame. It is disabled for readers who request reduced motion.
	const parallaxHero = document.querySelector('[data-article-parallax]');
	const reduceMotion = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)');
	if (parallaxHero && (!reduceMotion || !reduceMotion.matches)) {
		const parallaxImage = parallaxHero.querySelector('[data-article-parallax-image]');
		if (parallaxImage) {
			let scheduled = false;
			const updateParallax = function() {
				...
				// Cap the travel so the image remains calm and entirely within its frame.
				const offset = Math.max(-36, Math.min(36, (viewportCenter - heroCenter) * 0.08));
				parallaxImage.style.setProperty('--art-parallax-y', offset.toFixed(1) + 'px');
				scheduled = false;
			};
			...
			parallaxImage.addEventListener('load', requestParallaxUpdate, { once: true });
